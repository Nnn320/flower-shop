...
const props = defineProps({ addToCart: Function, setRoute: Function })
...
<template>
  <section class="catalog">
    ...
    <div class="search-bar">
      <input v-model="searchQuery" type="text" placeholder="搜索花束..." />
      <button v-if="searchQuery" @click="searchQuery = ''">清除搜索</button>
    </div>
    <div class="sort-bar">
    ...
    <div class="grid">
      <ProductCard v-for="p in filteredProducts" :key="p.id" :product="p" :add-to-cart="props.addToCart" />
    </div>
  </section>
</template>
...
.search-bar button { padding: 0.5rem 1rem; background: #ff9fb1; color: #fff; border: none; border-radius: 4px; cursor: pointer }
.sort-bar { margin: 0.5rem 0; display: flex; gap: 0.5rem }
.sort-bar button { padding: 0.4rem 0.8rem; background: #fff; border: 1px solid #ddd; border-radius: 4px; cursor: pointer }
